                    ...
                    <div class="card">
                        ...
                        <div class="card-body pt-7">
                            <form id="kt_subscriptions_export_form" class="form" method="POST" enctype="multipart/form-data"
                                action="{{ route('dms.projects.update',$project->id) }}" >
                                @csrf
                            ...
                            @method('PUT')
                                ...
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <select
                                        ...
                                        @forelse (\App\Models\DMS\Project::PROJECT_STATUS as $item)
                                            <option value="{{$item}}" @selected(old('project_status',$project->project_status) == $item )>{{ Str::ucfirst($item) }}</option>
                                        @empty
                                ...
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <!--begin::Label-->
                                    <label class="d-flex align-items-center fs-6 fw-bold mb-2">
                                        <span class=""><input type="checkbox" name="is_public" id="is_public" value="1" @checked(old('is_public',$project->is_public) == 1)>
                                            &nbsp;&nbsp; Is Public</span>
                                    </label>
                                ...
                                @php
                                // [$loop->index]['project_keyvalue']
                                //    $value= collect($project->projectDetails)->where('project_keyname',$key)->toArray()[$loop->index]['project_keyvalue'];
                                    // dd($value);
                                    // dd($value['value']);
                                @endphp
                                <div class="d-flex flex-column mb-8 fv-row">
                                    ...
                                    <!--end::Label-->
                                    <input type="text" class="form-control form-control-solid  {{ $errors->has($key) ? 'is-invalid' : '' }}" placeholder="{{ $value['value'] }}" name="{{ $key }}"
                                    value="{{ old($key, collect($project->projectDetails)->where('project_keyname',$key)->toArray()[$loop->index]['project_keyvalue']) }}"
                                    />
                                </div>
                                @empty
                                    &nbsp;
