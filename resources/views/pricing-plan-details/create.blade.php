...
<div class="card">
    <div class="card-header">
        {{ $pricingPlan->pricing_plan_name }} - {{ trans('global.create') }} {{ trans('crud.pricing_plan_details.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pricing-plans.pricing-plan-details.store", ['pricing_plan' => $pricingPlan]) }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pricing_plan_keyname') ? 'is-invalid' : '' }}" style="width: 100%;" name="pricing_plan_keyname" id="pricing_plan_keyname" required>
                    ...
                    @foreach(\App\Models\PricingPlanDetail::PRICING_PLAN_KEYNAMES as $key => $value)
                        <option value="{{ $key }}" {{ old('pricing_plan_keyname') == $key ? 'selected' : '' }}>{{ $value }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
                <label class="required" for="pricing_plan_value">{{ trans('crud.pricing_plan_details.fields.pricing_plan_value') }}</label>
                <input class="form-control {{ $errors->has('pricing_plan_value') ? 'is-invalid' : '' }}" type="text" name="pricing_plan_value" id="pricing_plan_value" value="{{ old('pricing_plan_value', '') }}" required>
            </div>
            <div class="form-group">
