                                    ...
                                    <tbody>
                                        @foreach ($levels as $level)
                                            <tr data-entry-id="{{ $level->id }}">
                                                ...
                                                <td>
                                                    @if($level->getFirstMediaUrl('Level') !== null)
                                                    ...
                                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-bold fs-7 w-125px py-4"
                                                        ...
                                                        <div class="menu-item px-3">
                                                            <form action="{{ route('admin.levels.destroy', $level->id) }}"
                                                                method="POST" id="frmDeleteLevel-{{ $level->id }}"
                                                                style="display: inline-block; width: 100%;">
                                                                <input type="hidden" name="_method" value="DELETE">
                                                                <input type="hidden" name="_token"
                                                                    value="{{ csrf_token() }}">
                                                                <a href="#" class="menu-link px-3"
                                                                    onclick="deleteGridRecord('frmDeleteLevel-{{ $level->id }}')">
                                                                    {{ trans('global.delete') }}
                                                                </a>
                                                            </form>
                                                        </div>
        ...
        $(function() {
                ...
                language: {
                    ...
                    @if ($levels->count())
                        info: '{{ trans('global.grid_pagination_count_status', [
                            'firstItem' => $levels->firstItem(),
                            'lastItem' => $levels->lastItem(),
                            'total' => $levels->total(),
                        ]) }}',
                    @endif
                },
            ...
            $('#dataTableSearch').on('keyup', function(e) {
                if (e.which == 13) { // if user press enter in search text input
                    ...
                    window.location.href = '{{ route('admin.levels.index') }}' + generateQueryString(
                        requestParameters);
                } else {
